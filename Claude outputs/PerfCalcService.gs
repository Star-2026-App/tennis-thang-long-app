...
/**
 * Tính Δstep cho 1 trận. Trả về null nếu trận KHÔNG đủ điều kiện tính (có
 * khách mời ở bất kỳ vị trí nào, không tra được Base cho đủ 4 người, hoặc
 * tổng game = 0/rỗng) — đúng yêu cầu mục 4 "Edge Cases" của tài liệu gốc:
 * bỏ qua, KHÔNG đoán mò, KHÔNG cộng dồn sai.
 *
...
function perfComputeMatchSteps_(match, S) {
  if (perfIsGuestName_(match.teamAP1Name) || perfIsGuestName_(match.teamAP2Name) ||
      perfIsGuestName_(match.teamBP1Name) || perfIsGuestName_(match.teamBP2Name)) {
    return null;
  }
  if (match.levelA1 == null || match.levelA2 == null ||
      match.levelB1 == null || match.levelB2 == null) {
    return null;
  }
  ...
  var totalA = match.levelA1 + match.levelA2;
  var totalB = match.levelB1 + match.levelB2;
