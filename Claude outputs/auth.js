...
function applyRolePermissions() {
    ...
    // (fix 06/09/2026 - lỗi Admin mở tab "Phong độ" ngay sau khi đăng nhập:
    // dropdown "Xem phong độ của..." trống, không chọn được ai) Nguyên nhân:
    // perfRenderMemberSelector_() (performance.js) trước đây CHỈ được gọi
    // ĐÚNG 1 LẦN, ngay lúc mở tab "Phong độ" lần đầu (activatePerfTab_) - nếu
    // lúc đó biến toàn cục `members` CHƯA kịp có danh sách thật (enterAppScreen_
    // nạp `members` bằng defaultFallbackMembers trước, dữ liệu thật chỉ về sau
    // qua updateStateFromCloud()/fetchCloudData() chạy BẤT ĐỒNG BỘ), dropdown sẽ
    // dựng ra rỗng và KHÔNG BAO GIỜ được dựng lại nữa trong suốt phiên đó, kể cả
    // sau khi `members` đã có đủ dữ liệu thật.
    // Gọi lại ở đây (đúng hook chạy sau MỌI lần dữ liệu/quyền thay đổi, đã có
    // sẵn cho perfSyncNavVisibility_ ở trên) để dropdown tự dựng lại đúng danh
    // sách ngay khi `members` thật đã về, không cần người dùng tải lại trang.
    // Hàm này tự return sớm nếu khối UI #tab-perf chưa được dựng (nhìn
    // document.getElementById('perf-member-selector')), nên gọi sớm cũng an
    // toàn, không throw lỗi.
    if (typeof perfRenderMemberSelector_ === 'function') {
