...
//
// GIỚI HẠN ĐÃ BIẾT (chấp nhận để giữ đơn giản — xem thêm cuối file): backfill
// CHỈ ghi PerformanceMonthlyClose (đủ để dựng lại đúng Điểm Base cuối cùng),
// KHÔNG ghi PerformanceHistory cho các tháng cũ — nghĩa là biểu đồ đường chi
// tiết từng trận trong tab "Phong độ" chỉ có dữ liệu đầy đủ TỪ THÁNG BẮT ĐẦU
// chạy job tự động hàng tháng (runPerfMonthlyRecalc_) trở đi, các tháng backfill
// chỉ để lại đúng 1 điểm chốt/tháng trong PerformanceMonthlyClose. Nếu về sau
// Star muốn có cả biểu đồ chi tiết cho lịch sử cũ, cần mở rộng thêm (không khó,
// chỉ là ghi thêm PerformanceHistory trong lúc preview — cố tình bỏ qua ở bản
// đầu để giữ độ phức tạp thấp nhất).
// ============================================================================
...
function perfPreviewBackfill_() {
  ...
  var result = { monthsProcessed: orderedMonths.length, rowsWritten: closeRows.length };
...
function perfApplyBackfill_() {
  ...
  var result = { membersUpdated: membersUpdated, rowsApplied: previewRowIndexes.length };
